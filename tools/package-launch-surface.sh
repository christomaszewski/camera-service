#!/usr/bin/env bash
# Package JUST the launch surface -- cam-up + the compose fragments + sensor_env + rigging.yaml --
# into a tarball you drop on a vehicle. No source tree, no Dockerfiles, no tests: the vehicle PULLS
# images from your registry and only needs these files to run cam-up. The file list IS the rig
# descriptor's `launch_surface` (the single source of truth), so the bundle can't drift from it.
#
#   tools/package-launch-surface.sh [output.tar.gz]        (default: dist/camera-launch.tar.gz)
#
# On the vehicle:
#   mkdir -p ~/cam && tar -xzf camera-launch.tar.gz -C ~/cam && cd ~/cam
#   export CAM_REGISTRY=<registry:5000>
#   ./cam-up /path/to/sensor.yaml pull && ./cam-up /path/to/sensor.yaml up -d
set -euo pipefail
REPO="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO"
OUT="${1:-dist/camera-launch.tar.gz}"
...
echo "  mkdir -p ~/cam && tar -xzf $(basename "$OUT") -C ~/cam && cd ~/cam"
echo "  export CAM_REGISTRY=<registry:5000>"
echo "  ./cam-up /path/to/sensor.yaml pull && ./cam-up /path/to/sensor.yaml up -d"
