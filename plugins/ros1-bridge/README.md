...
consumes the core's `application/x-gige-frame` shm endpoint and republishes each frame as
`sensor_msgs/Image`, stamping `header.stamp` from the per-frame hardware (PTP) timestamp in the 36-byte
header. The header contract is [`core-driver/gige_driver/transport.py`](../../core-driver/gige_driver/transport.py)
...
| graph | DDS / **rmw_zenoh** + a shared `rmw_zenohd` router | TCPROS + a shared **`roscore`** (`tools/roscore.sh` / `gige-up --roscore`) |
...
the **JP6 stack** (`gige-up --jp6`) — its GStreamer 1.20 publishes shm+header and skips unixfd; the
...
|---|---|---|
| `socket_path` | `/tmp/gige/frames` | the core's shm+header endpoint |
| `topic` | `image_raw` | output `sensor_msgs/Image` topic |
| `frame_id` | `camera` | `header.frame_id` (TF frame) |
| `encoding` | `""` (`$GIGE_ROS_ENCODING`) | Bayer label (e.g. `bayer_rggb8`); empty = `mono8`/`mono16` from the header. Auto-set by `gige-up` from `camera.pixel_format`. |

Color: the node publishes the raw mosaic labeled `bayer_*` (option A). `GIGE_DEBAYER=true` makes the
...
docker run --rm --network host --ipc=host -v gige_sock:/tmp/gige \
  -e GIGE_INSTANCE=cam_a -e ROS_MASTER_URI=http://localhost:11311 ros1-bridge
```
