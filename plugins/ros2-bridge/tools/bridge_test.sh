#!/usr/bin/env bash
# End-to-end test: dev producer (gige-dev, Ubuntu 22.04 / GStreamer 1.20 = JP6 userspace) + the
# ros2-bridge as a composable component over the shm+header transport -> sensor_msgs/Image. No Jetson
# required; validates the full camera -> timestamp -> shm+header -> GigeHeaderBridge -> ROS 2 path across
...
IMG="${GIGE_ROS2_IMAGE:-ros2-bridge}"

cleanup() { docker rm -f gige_producer gige_bridge gige_test_zenohd >/dev/null 2>&1; docker volume rm gige_sock >/dev/null 2>&1; }
trap cleanup EXIT
cleanup
docker volume create gige_sock >/dev/null

echo "== zenoh router (throwaway, bridge net so we can share its netns) =="
docker run -d --name gige_test_zenohd --entrypoint bash "$IMG" \
  -c 'source "/opt/ros/${ROS_DISTRO}/setup.bash"; exec ros2 run rmw_zenoh_cpp rmw_zenohd' >/dev/null
sleep 3

echo "== producer (gige-dev: shm+header, GStreamer 1.20) =="
...
for _ in $(seq 1 30); do docker run --rm -v gige_sock:/tmp/gige --entrypoint bash "$IMG" -c '[ -S /tmp/gige/frames ]' && break; sleep 0.5; done

echo "== bridge (GigeHeaderBridge component via launch, rmw_zenoh) =="
docker run -d --rm --name gige_bridge --network container:gige_test_zenohd --ipc=host -v gige_sock:/tmp/gige \
  -e RMW_IMPLEMENTATION=rmw_zenoh_cpp -e GIGE_PLATFORM=jp6 -e GIGE_INSTANCE=camera \
  -e GIGE_ROS_TOPIC=image_raw -e GIGE_FRAME_ID=camera "$IMG" >/dev/null
sleep 7
docker logs gige_bridge 2>&1 | grep -iE "Instantiate|consuming|ERROR|exception" | tail -3
...
VERIFY="$(mktemp /tmp/gige_verify.XXXX.py)"
cat > "$VERIFY" <<'PY'
...
print("PASS")
PY
docker cp "$VERIFY" gige_bridge:/tmp/verify.py >/dev/null; rm -f "$VERIFY"
docker exec gige_bridge bash -c '
