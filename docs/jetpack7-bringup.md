...
copy `core-driver/config/sensors/cam_gige.yaml` (the GigE template — chunk/PTP already wired), set
`gige.fake: false` + `gige.camera_id: <serial>`, and `recording.encoder: ffv1`. Then run the core
against the camera:
```bash
