...
def _blocking(appsrc) -> bool:
    """Whether the appsrc was built block=true. Read defensively: a real GstAppSrc always has the
    property; a test stub may not, and "unknown" must mean "enforce the bound" (the live-camera
    default), never a crash on the feed thread."""
    try:
        return bool(appsrc.get_property("block"))
...
class RecordingSession:
    ...
    def push(self, payload: bytes, pts: int, stamp, caps_str: Optional[str] = None) -> PushResult:
        ...
        with self.lock:
            ...
            # A block=true appsrc (a FINITE playback source: pipeline._feed_appsrc_props) is never
            # "full" here -- its push-buffer waits for room instead, which is the whole point of a
            # batch reprocess. Only a block=false feed (a live camera) needs the bound enforced by
            # hand, because appsrc itself never does.
            if not _blocking(self._appsrc):
