...
class _Appsrc:
    def __init__(self, order, max_bytes=0, level=0, flow=None, block=False):
        self.order = order
        self.props = {"max-bytes": max_bytes, "current-level-bytes": level, "block": block}
        self.pushed = []
        self.caps = []
        self.eos = 0
...
def _session(tmp, *, encoded=False, pop_msg="eos", flow=None, fail_state=False, on_error=None,
             max_bytes=0, level=0, block=False):
    order = []
    appsrc = _Appsrc(order, max_bytes, level, flow, block)
...
def test_a_blocking_feed_is_never_full_it_waits():
    # A FINITE (playback) source builds the session appsrc block=true (pipeline._feed_appsrc_props):
    # push-buffer waits for room, so the level pre-check must not turn a full queue into a drop --
    # a batch reprocess promises every frame lands. Same level as the drop test above; opposite result.
    with tempfile.TemporaryDirectory() as tmp:
        s, appsrc, bus, pipe, sc, order = _session(tmp, max_bytes=16, level=12, block=True)
        s.start(DROPS0)
        assert s.push(b"a" * 8, 100, _stamp(1)) is PushResult.OK
        assert s.frames == 1 and len(sc.rows) == 1 and len(appsrc.pushed) == 1
