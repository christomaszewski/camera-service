...
docker run --rm -v "$PWD/core-driver:/app" "$IMG" bash -c '
  ...
  python3 tests/test_pcapio.py
  python3 tests/test_uvcpcap.py
  python3 tests/test_replay.py
  python3 tests/test_keyframe.py
  ...
  gst-inspect-1.0 unixfdsink >/dev/null 2>&1 && SOCKET=/tmp/cam/unixfd
  touch "$SOCKET"   # bind() returns EADDRINUSE for ANY existing path, so this is faithful
  python3 main.py -c config/fake-camera.yaml >/tmp/core.log 2>&1 &
  CORE=$!
  sleep 5
  python3 tools/shm_probe.py --count 5 --timeout 5
  [ -e "$SOCKET.0" ] && { echo "FAIL: shmsink bound a suffixed path -- the stale-socket guard did not run"; exit 1; }
  grep -qE "removing a stale (plugin transport|unixfd transport) socket" /tmp/core.log \
